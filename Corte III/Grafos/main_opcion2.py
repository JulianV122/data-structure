...
def draw_network(users, relationships):
    # Calcular las coordenadas de los nodos
    node_positions = {}
    node_radius = 20
    x_spacing = WINDOW_WIDTH // (len(users) + 1)
    y_spacing = WINDOW_HEIGHT // (len(users) + 1)
    y_positions = [random.randint(y_spacing, WINDOW_HEIGHT - y_spacing) for _ in range(len(users))]
...
def main():
    ...
    while True:
        ...
        draw_network(users, relationships)
        pygame.display.flip()
        clock.tick(60)
