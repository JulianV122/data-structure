...
class facebook:
    def __init__(self,screen):
        ...
        self.combo2 = ComboBox(self.screen,["Red de amigos","Red de familiares","Red de comunidades"],self.combo_box_rect2,"Black","Arial Black",15,4,"White","White",40," Seleccione un usuario")
    ...
    def search_friends(self):
        user_select = self.list_users[self.combo1.getIndex()-1]
        print(self.combo1.getIndex()-1)
        print(f"Usuario {user_select}")
        for user_id, friends in self.data["relationships"].items():
                print("ID de usuario:", user_id)
                if self.combo1.getIndex()-1==user_id:
                    print("SII")
                    self.list_friends = friends
        print(self.list_friends)
